...
import { formatBalance, formatPercentage, timeAgo } from "@/lib/utils";
import {
  Table, TableBody, TableCell, TableHead, TableHeader, TableRow,
} from "@/components/ui/table";

interface EscDashboardData {
  snapshot: {
    id: number;
    date: string;
    timeSlot: string;
...
export default function EscDashboard() {
  const { data, isLoading } = useQuery<EscDashboardData>({
      ...
      <div className="flex flex-col items-center justify-center h-full gap-4 p-8">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-muted-foreground">
            Waiting for the first ESC snapshot. Data will appear within 5 minutes.
          </p>
        </div>
      </div>
    );
  ...
  const totalBalance = data.entries.reduce((s, e) => s + e.balance, 0);
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div>
          <h2 className="text-lg font-semibold flex items-center gap-2">
            <div className="w-6 h-6 rounded bg-purple-400/20 flex items-center justify-center">
              <span className="text-[10px] font-bold text-purple-400">ESC</span>
            </div>
            Elastos Smart Chain — Top 100
          </h2>
          <p className="text-xs text-muted-foreground mt-1">
            Top ELA holders on the Elastos Smart Chain (EVM sidechain)
          </p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <StatCard
          title="ESC Top 100 Balance"
          value={`${(totalBalance / 1000000).toFixed(2)}M`}
          subtitle="Total ELA in ESC top 100"
          icon={Wallet}
        ...
        <StatCard
          title="ESC Snapshots"
          value={data.stats.totalSnapshots}
          icon={Clock}
          iconColor="text-blue-400"
        />
        <StatCard
          title="Last Updated"
          value={data.snapshot ? timeAgo(data.snapshot.fetchedAt) : "—"}
          subtitle={data.snapshot ? `${data.snapshot.date} ${data.snapshot.timeSlot}` : undefined}
          icon={Clock}
                  ...
                  <TableRow key={entry.address} className="hover-elevate">
                    ...
                    <TableCell>
                      <div className="min-w-0">
                        {entry.label && (
                          <Badge variant="outline" className="text-xs mb-0.5 no-default-hover-elevate no-default-active-elevate">
                            {entry.label}
                          </Badge>
                        )}
                        <p className="font-mono text-xs text-muted-foreground truncate">{entry.address}</p>
                      </div>
                    </TableCell>
